      ...
      <section>
        ...
        <div className="grid gap-5">
          {upcomingEvents.map((event, i) => (
            <Link key={event.slug} href={`/events/${event.slug}`} className={`group relative rounded-2xl border border-border bg-card overflow-hidden card-hover gradient-border animate-fade-up delay-${(i + 1) * 100}`}>
              {event.coverImage && (
      ...
      <section className="mt-20">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {pastEvents.map((event, i) => (
            <Link key={event.slug} href={`/events/${event.slug}`} className={`group rounded-2xl border border-border bg-card overflow-hidden opacity-70 hover:opacity-100 card-hover transition-all animate-fade-up delay-${(i + 1) * 100}`}>
              {event.coverImage && (
                <div className="w-full aspect-[16/9] overflow-hidden">
                  <Image
                    src={event.coverImage}
